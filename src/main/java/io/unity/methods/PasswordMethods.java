...
public class PasswordMethods
{
    ...
    public void enter_password() {
        java_class.addMethod().
                ...
                setPublic().
                addAnnotation("io.unity.methods.MethodsData").setStringValue("method_id", "password_1");
    }

    @MethodsData(method_id = "password_2")
    public void clear_password() {
        java_class.addMethod().setName("clear_password_from_" + key_name).setBody("element.clear_password_field(\"" + key_name + "\");\n").setPublic().addAnnotation("io.unity.methods.MethodsData").setStringValue("method_id", "password_2");

    }

    @MethodsData(method_id = "password_3")
    public void clear_and_enter_password() {
        java_class.addMethod().setName("clear_text_and_enter_password_in_" + key_name).setBody("element.clear_and_enter_in_password_field(\"" + key_name + "\",text_to_enter);").setParameters("String text_to_enter").setPublic().addAnnotation("io.unity.methods.MethodsData").setStringValue("method_id", "password_3");
    }


    @MethodsData(method_id = "password_4")
    public void verify_password_is_present() {
        java_class.addMethod().setName("verify_" + key_name + "_is_present_on_page").setBody("verify.element_is_present(\"" + key_name + "\");").setPublic().addAnnotation("io.unity.methods.MethodsData").setStringValue("method_id", "password_4");
    }
